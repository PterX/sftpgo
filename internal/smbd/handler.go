...
// 自己实现一个fileTree，带文件操作的，与provider转化一下
...
func (fs *PassthroughFS) Mkdir(p string, mode int) (*vfs.Attributes, error) {
	fs.UpdateLastActivity()

	return nil, fs.CreateDir(p, true)
}
...
func (fs *PassthroughFS) Rmdir(node vfs.VfsNode, name string) error {
	fs.UpdateLastActivity()
	// TODO get node name and join
	return fs.BaseConnection.RemoveDir(name)
}
...
func (fs *PassthroughFS) Rename(from vfs.VfsHandle, to string, flags int) error {
	...
	return fs.BaseConnection.Rename(open.path, path.Join(fs.rootPath, to))
}
